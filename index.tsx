...
function initNativeCarousel() {
    ...
    const updateCarousel = () => {
        ...
        const numPages = totalSlides - slidesPerView + 1;

        // Create or update dots based on the number of pages
        if (dotsContainer.children.length !== numPages && numPages > 1) {
            dotsContainer.innerHTML = Array.from({ length: numPages }, (_, i) => 
                `<button class="carousel-dot" data-index="${i}" aria-label="Ir para depoimento ${i + 1}"></button>`
            ).join('');
        } else if (numPages <= 1) {
